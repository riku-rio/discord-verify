...
> **Development package manager:** `pnpm` (Node.js), `uv` (Python)
...
16. Use `pnpm` for development and workspace management.
17. Use `uv` for Python environment and dependency management.
18. Require Python 3.10 or later for the Python adapter.
...
* `uv` as the preferred environment and dependency manager.
...
* `pnpm` is the development package manager for workspaces and dependency management.
* `uv` is used for Python virtual-environment creation and dependency management.
* Python 3.10+ is required for the Python adapter.
* Published packages remain compatible with npm for consumers.
...
* Python version (3.10+).
* Package-manager availability (`pnpm`, `npm`, `uv`, `pip`, `Poetry`).
* `uv` availability for Python environment management.
...
Python environment resolution should prefer:

1. Explicit configuration.
2. Active virtual environment.
3. `uv`-managed environment.
4. Poetry-managed environment.
5. Project `.venv`.
6. System Python.
